...
function _the_logo() {
	$tag = is_home() ? 'h1' : 'div';
	echo '<' . $tag . ' class="logo"><a href="' . get_bloginfo('url') . '" title="' . get_bloginfo('name') .(get_bloginfo('description') ? _get_delimiter() . get_bloginfo('description') : ''). '"><img src="'._hui('logo_src').'">' . get_bloginfo('name') . '</a></' . $tag . '>';
}
